...
(define-read-only (get-investment-tokens
    (project-id uint)
    (investor principal)
  )
  (map-get? investment-tokens {
...
(define-read-only (get-revenue-distribution
    (project-id uint)
    (distribution-id uint)
  )
  (map-get? revenue-distributions {
    project-id: project-id,
    distribution-id: distribution-id,
  })
)

(define-read-only (get-revenue-claim
    (project-id uint)
    (distribution-id uint)
    (claimer principal)
  )
  (map-get? revenue-claims {
    project-id: project-id,
    distribution-id: distribution-id,
    claimer: claimer,
  })
...
(define-private (calculate-tokens-for-investment
    (investment-amount uint)
    (total-project-funding uint)
    (total-project-tokens uint)
  )
...
(define-private (calculate-revenue-share
    (tokens-owned uint)
    (total-project-tokens uint)
    (total-revenue uint)
  )
...
(define-public (mint-project-tokens
  ...
  (let (
    ...
    (map-set project-tokens { project-id: project-id } {
...
(define-public (distribute-investment-tokens
  ...
  (let (
    ...
    (asserts! (<= new-distributed (get total-tokens project-token-data))
...
(define-public (vote-on-proposal
  ...
  (let (
      ...
      (new-yes-votes (if vote
        (+ (get yes-votes proposal-data) user-voting-power)
        (get yes-votes proposal-data)
      ))
      (new-no-votes (if vote
        (get no-votes proposal-data)
        (+ (get no-votes proposal-data) user-voting-power)
      ))
    )
    (asserts! (not (var-get contract-paused)) err-unauthorized)
    (asserts! (> user-voting-power u0) err-insufficient-balance)
    (asserts! (<= current-time (get voting-end proposal-data))
...
(define-public (claim-revenue-share
  ...
  (let (
      (distribution-data (unwrap! (get-revenue-distribution project-id distribution-id)
        err-not-token-owner
      ))
      (investment-data (unwrap! (get-investment-tokens project-id tx-sender) err-not-token-owner))
    ...
    (asserts! (is-none (get-revenue-claim project-id distribution-id tx-sender))
      err-already-voted
    )
